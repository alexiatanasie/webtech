...
//get employees?simplifed=true&filter=im
...
app.get('/employees',async(req,res,next)=>{
    try{
        ...
        if(req.query.simplified === 'true'){
            query.attribute =['firstName','lastName']
        }
        if(req.query.filter){
            query.where={
...
app.put('/employees/:eid',async(req,res,next)=>{
    ...
    console.log(result)
        if(result.shift() !==0){
        //shift get frist val of the array
...
app.use((err,req,res,next)=>{
    console.warn(err) //log doesnt have enought prior
